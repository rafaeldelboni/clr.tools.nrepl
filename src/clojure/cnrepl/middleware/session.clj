(ns cnrepl.middleware.session
  ...
  (:import
   (clojure.lang Compiler+CompilerException LineNumberingTextReader)               ;;; Compiler$CompilerException LineNumberingPushbackReader
                                                                                   ;;; (java.io Reader)
   (clojure.lang AtomicLong)                                                       ;;; (java.util.concurrent.atomic AtomicLong)
    (System.Threading ThreadStart Thread ThreadState ThreadInterruptedException)   ;;; (java.util.concurrent BlockingQueue LinkedBlockingQueue SynchronousQueue
...
;;; DM ;;;;;;;;;
;; In CLR, we don't have ThreadFactory, context loaders, an ExecutorService, etc.
;; analyzing what all this does, what we need to provide is:
;;  Given an id and two ThreadStart delegates, 
;;    create a single ThreadStart delegate which runs the two given delegates in sequence, 
;;    using the ThreadPool.
;;  (I won't know until later, but maybe we will only pass an in IFn???)
                  ...
                  (locking input-queue
                    (loop [i off]
                      (cond
                        (>= i (+ off len))
                        (+ off len)
                        (> (.Count input-queue) 0)                                            ;;; (.peek input-queue) 
                          (do (aset-char buf i (char (.Take input-queue)))                    ;;;  .take
                            (recur (inc i)))
                        :else
                        i))))
        reader (LineNumberingTextReader.                                                      ;;; LineNumberingPushbackReader.
                 (proxy [System.IO.TextReader] []                                             ;;; Reader
                   (Dispose [disposing] (when disposing (.Dispose input-queue)))              ;;; (close [] (.clear input-queue))
...
(defn session-exec
  ...
  (let [                                                                               ;;; cl (clojure.lang.DynamicClassLoader.
        ...
        main-loop #(try
                     (loop []
                       (let [[exec-id runnable ack] (.Take queue)
                             wc-runnable (gen-delegate System.Threading.WaitCallback [_] runnable)
        ...
        spawn-thread #(doto (Thread. (gen-delegate ThreadStart [] (main-loop)))                        ;;; (Thread. main-loop (str "nREPL-session-" id))
                        ...
                        (.set_Name  (str "nREPL-session-" id))                                        ;;;  (.setContextClassLoader cl)
                        .Start)]                                                                      ;;;  .start
    ...
    {:interrupt (fn [exec-id]
     ...
     :exec (fn [exec-id runnable ack]
             (.Add queue [exec-id runnable ack]))}))                                                                              ;;; .put
...
(defn add-stdin
  ...
  (fn [{:keys [op stdin session transport] :as msg}]
    (cond
      (= op "eval")
        (let [in (-> (meta session) ^LineNumberingTextReader (:stdin-reader))]                 ;;; LineNumberingPushbackReader
        (binding [*skipping-eol* true]
          (clojure.main/skip-if-eol in))
        (h msg))
      (= op "stdin")
      (let [q (-> (meta session) ^|System.Collections.Concurrent.BlockingCollection`1[System.Object]| (:input-queue))]      ;;; ^LinkedBlockingQueue
        (if (empty? stdin)
          (.Add q -1)                                                                           ;;; .put
          (locking q
            (doseq [c stdin] (.Add q c))))                                                      ;;; .put
        (t/send transport (response-for msg :status :done)))
      :else
      (h msg))))
