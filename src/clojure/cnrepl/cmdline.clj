...
(defn- clean-up-and-exit
  ...
  (Environment/Exit status))                                                   ;;; System/exit
...
(defn- build-handler
  ...
  [middleware]
  (apply cnrepl.server/default-handler (->mw-list middleware)))

(defn- ->int [x]
  (cond
    (nil? x) x
    (number? x) x
    :else (Int32/Parse x)))                                               ;;; Integer/parseInt
...
(defn -main
  [& args]
  (try
    #_(set-signal-handler! "INT" handle-interrupt)                           ;;;TODO:  DO this with catching cancel on the console.
